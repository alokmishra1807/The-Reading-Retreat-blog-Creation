...
export const allBlogs = TryCatch(async(req,res)=>{
...
if(searchQuery && category){
    blogs = await sql`SELECT * FROM blogs WHERE (title ILIKE ${"%" + searchQuery + "%"} OR description ILIKE ${
        "%" + searchQuery + "%"
...
export const getSingleBlog = TryCatch(async(req,res)=>{
    ...
    if(cached){
         console.log("Serving singleBlog from Redis Cache");
    res.json(JSON.parse(cached));
    return;
    }
