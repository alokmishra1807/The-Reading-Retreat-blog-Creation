...
// export const createBlog = TryCatch(async (req: AuthenticatedRequest, res) => {
//   console.time("🕒 Total Blog Creation Time");
...
//     console.timeEnd("🕒 Total Blog Creation Time");
//     return;
//   }
//   console.timeEnd("📁 File Check");

//   console.time("📦 Buffer Generation");
//   const fileBuffer = getBuffer(file);

//   if (!fileBuffer || !fileBuffer.content) {
//     res.status(400).json({ message: "Failed to generate buffer" });
//     console.timeEnd("📦 Buffer Generation");
//     console.timeEnd("🕒 Total Blog Creation Time");
//     return;
//   }
//   console.timeEnd("📦 Buffer Generation");
...
//   console.time("📤 RabbitMQ Job");
//   await invalidateCacheJob(["blogs:*"]);
//   console.timeEnd("📤 RabbitMQ Job");
...
title without any additional text, formatting, or symbols: "${text}"`;
  ...
  async function main() {
    ...
    result = rawText
      ...
      .replace(/[\r\n]+/g, "") // removes all line breaks (\r and \n)
